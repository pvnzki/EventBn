...
class PostServiceRabbitMQPublisher {
  ...
  async handleConnectionError() {
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(`[POST-RABBITMQ] Attempting reconnection ${this.reconnectAttempts}/${this.maxReconnectAttempts} in ${delay}ms`);
  ...
  async publishEvent(routingKey, eventType, data, options = {}) {
    try {
      ...
      console.log(`[🚀] [POST-RABBITMQ] Published event '${eventType}' with routing key '${routingKey}'`);
      return true;

    } catch (error) {
      console.error(`[❌] [POST-RABBITMQ] Failed to publish event '${eventType}':`, error.message);
      return false;
    }
  }
  ...
  async publishAnalyticsEvent(eventType, analyticsData, options = {}) {
    return this.publishEvent('post.analytics', eventType, analyticsData, options);
  }

  // Specific event methods for common use cases
  async publishPostCreated(postData, options = {}) {
    return this.publishPostEvent('POST_CREATED', {
  ...
  async publishPostLiked(postData, likeData, options = {}) {
    return this.publishSocialEvent('POST_LIKED', {
      post_id: postData.post_id,
  ...
  async publishPostUnliked(postData, unlikeData, options = {}) {
    return this.publishSocialEvent('POST_UNLIKED', {
      post_id: postData.post_id,
      post_user_id: postData.user_id,
  ...
  async publishCommentCreated(commentData, options = {}) {
    return this.publishSocialEvent('COMMENT_CREATED', {
  ...
  async publishUserFollowed(followData, options = {}) {
    return this.publishSocialEvent('USER_FOLLOWED', {
  ...
  async publishEngagementMetrics(metricsData, options = {}) {
    return this.publishAnalyticsEvent('ENGAGEMENT_METRICS', {
  ...
  async publishFeedInteraction(interactionData, options = {}) {
    return this.publishAnalyticsEvent('FEED_INTERACTION', {
...
module.exports = {
  ...
  publishAnalyticsEvent: (eventType, analyticsData, options) => 
    postRabbitMQPublisher.publishAnalyticsEvent(eventType, analyticsData, options),
