...
const authenticateUser = (req, res, next) => {
  try {
    ...
    return res.status(401).json({ success: false, error: 'Invalid or expired token', message: e.message });
  }
};
...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    let result;
    try {
      if (!coreService?.auth?.login) throw new Error('Auth service unavailable');
      result = await coreService.auth.login({ email, password });
    } catch (e) {
      return res.status(401).json({ success: false, error: e.message || 'Login failed', message: e.message || 'Login failed' });
    }
    res.status(200).json({
      success: true,
...
router.post("/auth/register", async (req, res) => {
  try {
    ...
    if (!coreService?.auth?.register) {
      return res.status(503).json({ success: false, error: 'Auth service unavailable', message: 'Auth service unavailable' });
    }
    let result;
    try {
      result = await coreService.auth.register({ name, email, password, phone_number, profile_picture });
    } catch (e) {
      return res.status(400).json({ success: false, error: e.message || 'Registration failed', message: e.message || 'Registration failed' });
    }
    res.status(201).json({
      success: true,
...
router.get("/events", async (req, res) => {
  try {
    ...
    if (coreService?.events?.getAllEvents) {
      try {
        const result = await coreService.events.getAllEvents({ category, location, page: parseInt(page), limit: parseInt(limit) });
        ...
        console.warn("[API] getAllEvents failed, using fallback:", innerErr.message);
      }
    }
...
async function getUnifiedEvents() {
  if (coreService?.events?.getAllEvents) {
    try {
      const result = await coreService.events.getAllEvents({ page: 1, limit: 500 });
  ...
  try {
    ...
    if (req.query.view === 'summary') {
      featured = featured.map(e => ({
        ...
        image: e.cover_image_url || e.banner_url || placeholderImageForCategory(e.category)
      }));
    }

    res.json({ success: true, data: featured, events: featured, count: featured.length, service: 'core-service' });
  } catch (e) {
    res.status(200).json({ success: true, data: [], fallback: true, service: 'core-service', error: e.message });
  }
});
...
router.get('/events/search/:query', async (req, res) => {
  try {
    ...
    let results = all.filter(ev => [ev.title, ev.description, ev.category]
      .filter(Boolean)
    ...
    res.json({ success: true, data: results, events: results, count: results.length, service: 'core-service' });
  } catch (e) {
    res.status(200).json({ success: true, data: [], fallback: true, service: 'core-service', error: e.message });
  }
});
...
router.get('/events/:eventId/attendees', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: [], service: 'core-service', note: 'Attendees endpoint stub (microservice mode)' });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to fetch attendees', message: e.message });
  }
});
...
router.get('/events/:eventId/seatmap', async (req, res) => {
  try {
    ...
    if (coreService?.events?.getEventById) {
      try { event = await coreService.events.getEventById(eventId); } catch(_) {}
    }
    if (!event) {
    ...
    if (Array.isArray(seatmap) && memory?.seatLocks) {
      ...
      if (locked) {
        seatmap = seatmap.map(seat => ({
          ...seat,
            available: seat.available === false ? false : !locked.has(String(seat.id ?? seat.label))
        }));
      }
    }
    res.json({ success: true, data: seatmap, eventId, service: 'core-service' });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to fetch seatmap', message: e.message });
  }
});
...
router.get('/events/:eventId/booked-seats', async (req, res) => {
  try {
    ...
    res.json({ success: true, data, count: data.length, service: 'core-service' });
  } catch (e) {
    res.status(500).json({ success: false, message: 'Failed to fetch booked seats', error: e.message });
  }
});
...
router.post('/payments', authenticateUser, express.json(), async (req, res) => {
  try {
    ...
    const { event_id, selected_seats, selectedSeatData, payment_method = 'card' } = req.body || {};
    if (!event_id || !Array.isArray(selected_seats) || selected_seats.length === 0) {
      return res.status(400).json({ success: false, error: 'event_id and selected_seats[] required', message: 'event_id and selected_seats[] required' });
    }
    // Fetch event & current booked seats
    const event = await prisma.event.findUnique({ where: { event_id: parseInt(event_id) }, select: { event_id: true, seat_map: true, title: true } });
    if (!event) return res.status(404).json({ success: false, error: 'Event not found', message: 'Event not found' });

    // Collect already booked seats (pending/completed payments)
    const existingTickets = await prisma.ticketPurchase.findMany({
      where: { event_id: event.event_id, payment: { status: { in: ['pending','completed'] } } },
      select: { seat_id: true, seat_label: true }
    });
    const taken = new Set(existingTickets.map(t => String(t.seat_id || t.seat_label)));
    const conflicts = selected_seats.filter(s => taken.has(String(s)));
    if (conflicts.length) {
      return res.status(409).json({ success: false, error: 'Some seats already booked', conflicts });
    }
    ...
    const ticketRows = selected_seats.map((label, idx) => {
      const seatInfo = selectedSeatData && selectedSeatData[idx] ? selectedSeatData[idx] : null;
      const priceCents = seatInfo?.price_cents ? parseInt(seatInfo.price_cents) : seatInfo?.price ? Math.round(parseFloat(seatInfo.price) * 100) : 1000;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const takenNow = new Set(concurrent.map(t => String(t.seat_id || t.seat_label)));
      const conflictsNow = selected_seats.filter(s => takenNow.has(String(s)));
      if (conflictsNow.length) {
        throw new Error('Seat(s) just booked by another user: ' + conflictsNow.join(','));
      }
      ...
      for (const row of ticketRows) {
        const seatIdParsed = row.seatInfo?.id ? parseInt(row.seatInfo.id) : null;
      ...
      if (event.seat_map) {
        ...
        await tx.event.update({ where: { event_id: event.event_id }, data: { seat_map: seatMapObj } });
      }
    ...
    const eventInfo = await prisma.event.findUnique({
      ...
      select: { title: true, cover_image_url: true, start_time: true, venue: true, location: true }
    });
    res.status(201).json({
      success: true,
    ...
    if (error.message && error.message.startsWith('Seat(s) just booked')) {
      return res.status(409).json({ success: false, error: error.message, message: error.message });
    }
    res.status(500).json({ success: false, error: 'Failed to create payment record', message: error.message });
  }
});
...
router.get('/payments/my-payments', authenticateUser, async (req, res) => {
  try {
    const userId = parseInt(req.userId);
    const payments = await prisma.payment.findMany({ where: { user_id: userId }, orderBy: { payment_date: 'desc' } });
    res.json({ success: true, payments });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to fetch payments', message: e.message });
  }
});
...
router.get('/payments/:payment_id', authenticateUser, async (req, res) => {
  try {
    ...
    if (!payment || payment.user_id !== userId) return res.status(404).json({ success: false, error: 'Payment not found' });
    res.json({ success: true, payment });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to fetch payment', message: e.message });
  }
});
...
router.get('/tickets/my-tickets', authenticateUser, async (req, res) => {
  try {
    ...
    const tickets = await prisma.ticketPurchase.findMany({
      ...
      include: { event: { select: { title: true, cover_image_url: true, start_time: true, venue: true, location: true } } }
    });
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch tickets', message: e.message });
  }
});
...
router.get('/tickets/:ticketId', authenticateUser, async (req, res) => {
  try {
    ...
    const ticket = await prisma.ticketPurchase.findUnique({ where: { ticket_id: ticketId }, include: { event: { select: { title: true, cover_image_url: true, start_time: true, venue: true, location: true } } } });
    if (!ticket || ticket.user_id !== userId) return res.status(404).json({ success: false, error: 'Ticket not found' });
    res.json({ success: true, ticket: { ...ticket, price: Number(ticket.price), event: ticket.event } });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to fetch ticket', message: e.message });
  }
});
...
router.get('/tickets/qr/:qrCode', authenticateUser, async (req, res) => {
  try {
    ...
    const ticket = await prisma.ticketPurchase.findFirst({ where: { qr_code: qrCode, user_id: userId }, include: { event: { select: { title: true, cover_image_url: true, start_time: true, venue: true, location: true } } } });
    if (!ticket) return res.status(404).json({ success: false, error: 'Ticket not found' });
    res.json({ success: true, ticket: { ...ticket, price: Number(ticket.price), event: ticket.event } });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to fetch ticket by QR', message: e.message });
  }
});

// GET /tickets/by-payment/:paymentId (parity first ticket)
router.get('/tickets/by-payment/:paymentId', authenticateUser, async (req, res) => {
  try {
    const userId = parseInt(req.userId);
    const { paymentId } = req.params;
    const ticket = await prisma.ticketPurchase.findFirst({ where: { payment_id: paymentId, user_id: userId }, include: { event: { select: { title: true, cover_image_url: true, start_time: true, venue: true, location: true } } } });
    if (!ticket) return res.status(404).json({ success: false, error: 'Ticket not found' });
    res.json({ success: true, ticket: { ...ticket, price: Number(ticket.price), event: ticket.event } });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to fetch ticket by payment', message: e.message });
  }
});
...
router.put('/tickets/:ticketId/attend', authenticateUser, async (req, res) => {
  try {
    ...
    const ticket = await prisma.ticketPurchase.findUnique({ where: { ticket_id: ticketId }, include: { event: { select: { title: true, cover_image_url: true, start_time: true, venue: true, location: true } } } });
    if (!ticket || ticket.user_id !== userId) return res.status(404).json({ success: false, error: 'Ticket not found' });
    if (ticket.attended) return res.json({ success: true, message: 'Already marked attended', ticket: { ...ticket, price: Number(ticket.price) } });
    const updated = await prisma.ticketPurchase.update({ where: { ticket_id: ticketId }, data: { attended: true } });
    res.json({ success: true, message: 'Ticket marked as attended', ticket: { ...updated, price: Number(updated.price), event: ticket.event } });
  } catch (e) {
    res.status(500).json({ success: false, error: 'Failed to update ticket attendance', message: e.message });
  }
});
...
router.get("/users/profile", async (req, res) => {
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { user_id: userId },
      select: { user_id: true, name: true, email: true, phone_number: true, profile_picture: true, role: true, is_active: true, is_email_verified: true, created_at: true, updated_at: true }
    });
...
router.put("/users/profile", async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Profile updated successfully', user, service: 'core-service' });
  } catch (error) {
