...
module.exports = {
  ...
  // Update user profile (used by Mobile App). This updates the primary DB (prisma)
  // and publishes a USER_UPDATED event via RabbitMQ. Publishing failures are
  // logged but do not fail the DB update.
  async updateUserProfile(userId, profileData) {
    try {
      const updateData = {};

      // Map incoming profile fields to prisma user fields. Add mappings as needed.
      ...
      // Publish USER_UPDATED event (best-effort). Do not crash the update if publishing fails.
      try {
        ...
        // Log publishing error but don't throw - the DB update succeeded.
        console.error("Failed to publish USER_UPDATED event:", pubErr);
      }
