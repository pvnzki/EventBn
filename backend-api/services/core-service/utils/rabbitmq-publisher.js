...
class RabbitMQPublisher {
  ...
  async handleConnectionError() {
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(
        `[RABBITMQ] Attempting reconnection ${this.reconnectAttempts}/${this.maxReconnectAttempts} in ${delay}ms`
      );
  ...
  async publishEvent(routingKey, eventType, data, options = {}) {
    try {
      ...
      console.log(
        `[🚀] [RABBITMQ] Published event '${eventType}' with routing key '${routingKey}'`
      );
      return true;
    } catch (error) {
      console.error(
        `[❌] [RABBITMQ] Failed to publish event '${eventType}':`,
        error.message
      );
