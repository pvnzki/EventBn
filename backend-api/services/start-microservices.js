...
const log = (service, message, type = "info") => {
  ...
  console.log(
    `${colors[type]}[${timestamp}] [${service.toUpperCase()}] ${message}${
      colors.reset
    }`
  );
};
  ...
  return new Promise((resolve, reject) => {
      ...
      const req = http.request(options, (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode >= 200 && res.statusCode < 400) {
            log(
              "SYSTEM",
              `✅ Service at ${url} is ready (${res.statusCode})`,
              "success"
            );
            resolve(true);
          } else {
    ...
    const handleRetry = (error) => {
      ...
      if (retries >= maxRetries) {
        reject(
          new Error(
            `Service at ${url} not ready after ${maxRetries} attempts. Last error: ${error}`
          )
        );
      } else {
        if (retries === 1 || retries % 10 === 0) {
          log(
            "SYSTEM",
            `⏳ Waiting for service (attempt ${retries}/${maxRetries})...`,
            "info"
          );
        }
...
`);
  ...
  try {
    ...
    try {
      ...
      log(
        "SYSTEM",
        `Core-service health check failed: ${error.message}`,
        "error"
      );
      throw error;
    }
    ...
    try {
      ...
      log(
        "SYSTEM",
        `Post-service health check failed: ${error.message}`,
        "error"
      );
      throw error;
    }
