...
class PerformanceMonitor {
  ...
  Map<String, dynamic> getMetrics() {
    return {
      ...
      'min_response_time':
          _responseTimes.isEmpty ? 0 : _responseTimes.reduce(min),
      'max_response_time':
          _responseTimes.isEmpty ? 0 : _responseTimes.reduce(max),
  ...
  double _calculateOverallCacheHitRate() {
    ...
    final totalAttempts =
        _cacheAttempts.values.fold(0, (sum, attempts) => sum + attempts);
  ...
  Map<String, int> _getResponseTimeDistribution() {
    ...
    for (final time in _responseTimes) {
      if (time <= 100) {
        ...
        distribution['medium_100_500ms'] =
            distribution['medium_100_500ms']! + 1;
      } else if (time <= 1000) {
        distribution['slow_500_1000ms'] = distribution['slow_500_1000ms']! + 1;
      } else {
        distribution['very_slow_1000ms+'] =
            distribution['very_slow_1000ms+']! + 1;
      }
    }
  ...
  List<String> _generateRecommendations() {
    ...
    if (avgResponseTime > 500) {
      recommendations.add(
          'Consider optimizing API responses or improving network conditions');
    }

    if (cacheHitRate < 70) {
      recommendations
          .add('Improve caching strategy to increase cache hit rate');
    }

    if (memoryUsage > 200) {
      recommendations
          .add('Monitor memory usage and consider reducing cache sizes');
    }

    if (_activeConnections > 10) {
      recommendations
          .add('Consider connection pooling to reduce active connections');
    }
