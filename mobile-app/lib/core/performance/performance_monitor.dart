...
class PerformanceMonitor {
  ...
  void recordCacheMiss(String cacheType) {
    _counters['${cacheType}_misses'] =
        (_counters['${cacheType}_misses'] ?? 0) + 1;
  ...
  void recordError(String errorType, String errorMessage) {
    ...
    _counters['${errorType}_count'] =
        (_counters['${errorType}_count'] ?? 0) + 1;
    _addEvent('error', {
      'type': errorType,
  ...
  String getPerformanceReport() {
    ...
    if (cacheStats.isNotEmpty) {
      ...
      for (final entry in cacheStats.entries) {
        ...
        buffer.writeln(
            '  ${entry.key}: ${stats['hit_rate']}% hit rate (${stats['hits']}/${stats['total_requests']})');
      }
    }
